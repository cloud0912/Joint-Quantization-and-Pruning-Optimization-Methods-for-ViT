...
def generate_model_params_set(layers=12, heads=12, num_groups=12):
    params_set = set()
    for layer in range(layers):
        ...
        for group in range(num_groups):
            params_set.update({    
                f"ffn{layer}_group_{group}_input1",
                f"ffn{layer}_group_{group}_input2",
            })
...
class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, layers, dropout = 0., num_groups=3):
        super().__init__()
    ...
    def forward(self, x):
        ...
        for i in range(num_groups):
            ...
            x[:, group_start:group_end, :] = _quantization_activations(x[:, group_start:group_end, :], False, common + "_group_" + str(i) + "_input1")
        x = self.FFN2(x)
        for i in range(num_groups):
            group_start = i * group_size
            group_end = (i + 1) * group_size
            x[:, group_start:group_end, :] = _quantization_activations(x[:, group_start:group_end, :], False, common + "_group_" + str(i) + "_input2")
        return x
