...
class Attention(nn.Module):
    ...
    def forward(self, x, mask = None):
        ...
        # 量化 QKV
#         q = _quantization_activations(q, False, common + "_q")
#         k = _quantization_activations(k, False, common + "_k")
#         v = _quantization_activations(v, False, common + "_v")

        dots = torch.einsum('bhid,bhjd->bhij', q, k) * self.scale  ## （batchsize, heads, len_q, len_k）,其中(len_q = len_k = num_batches+1)

        # 量化 attention
        dots = _quantization_activations(dots, False, common + "_attn")
        ...
        # 量化输出
        out = _quantization_activations(out, False, common + "_out")
