...
class Attention(nn.Module):
    def __init__(self, dim, dim1, layers, heads = 8, dropout = 0.):
        super().__init__()
        ...
        if cfg is not None:
            for i, num in enumerate(cfg):
                self.layers.append(nn.ModuleList([
                    Residual(PreNorm(dim, Attention(dim, num[0], heads = heads, dropout = dropout, layers = i))),
                    Residual(PreNorm(dim, FeedForward(dim, num[1], dropout = dropout, layers = i)))
                ]))
        else:
            for i in range(depth):
                self.layers.append(nn.ModuleList([
                    Residual(PreNorm(dim, Attention(dim, dim, heads = heads, dropout = dropout, layers = i))),
                    Residual(PreNorm(dim, FeedForward(dim, dim, dropout = dropout, layers = i)))
                ]))    
    def forward(self, x, mask = None):
...
class ViT_slim(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, cfg=None, channels = 3, dropout = 0., emb_dropout = 0., k=8):
        super().__init__()
